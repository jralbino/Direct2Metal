...
> Estado: **V197** — **367 ms/frame sostenidos** (201 frames a 366-367 ms, 66.6 °C, `arm=1000MHz thr=0x0`); padding explícito → el fast path cubre el 100 % de las posiciones (435 → 367, −15.6 %, bit-exacto). V196 — **435 ms/frame sostenidos** (301 frames, hasta 66 °C, `arm=1000MHz thr=0x0`). V196 arregla la caída del reloj ARM a 600 MHz a los ~60 s del boot que degradaba a 705 ms (+38 % de throughput real, sin tocar el grafo). V195 — P8 stride-2 + acumuladores `noinline`: 504 → 435, −13.7 %, bit-exacto. V194 — instrumentación `[SOC]` (reloj real + throttle + temperatura por mailbox), que estableció la línea base **504 ms @ 1000 MHz** reproducible a ±0 ms. El 492 ms de V192 se midió sin registrar condiciones y no se reprodujo; **no es throttling térmico** (falsado con medición directa, ver fila V194). V193 — **Winograd F(2×2,3×3) implementado, validado y CERRADO** (verdicto negativo, ver fila V193). Geometría vigente desde **V192**, campo de visión completo (entrada no cuadrada 320×192). YOLOv8n es fully-conv → mismos pesos. Golden = frame de cámara recapturado (`clock` 56 %, huella `[ABS]` PASS 16/16). En GitHub `main` (hasta `3ad18eb`; V193 aún sin push).
>
...
| **V195** | **P8 para stride-2 + los acumuladores como funciones hoja: 504 → 435 ms (−13.7 %).** Dos cambios en `bsp/multicore.cpp`, ambos bit-exactos (`[ABS]` 16/16 PASS, `[DET]` sin cambio), medidos por separado con el nuevo kill-switch `EXTRA_DEF=-DD2M_NO_S2P8`. **(1) El fast path P8 no existía para stride 2** — las gates de `conv2d_partial` y `conv2d_partial_8ch` exigían `stride == 1`, así que las 6 conv3×3 stride-2 (L1/L3/L5/L7/L16/L19, **87 ms = 17 % del frame**) corrían el camino genérico que re-lee el bloque de pesos por cada posición de salida. Generalizado el sub-rectángulo seguro a `(stride,pad)` cualquiera — `oy ≥ ceil(p/s)`, `oy ≤ (H_in−K+p)/s`, que para s=1,p=1,K=3 reproduce exactamente el `[1,H_out−1)` de V191 y para s=2 elimina el anillo inferior/derecho (L1: límite 48 = H_out). **87.1 → 55.7 ms (−31.4).** **(2) El cuerpo acumulador extraído a `p8_accum_{4,8}ch<S>`, templates `noinline`** especializados en el stride: **−37 ms adicionales, todos en capas stride-1** (head 185 → 164, P3 head 112.0 → 97.7, L4 30.4 → 27.0, L12 22.4 → 20.8). *El mecanismo no queda establecido*: **no** es el efecto de stack frame de V193 — el frame de `conv2d_partial_8ch` apenas baja de 1760 a 1680 B. Lo que sí cambia es la forma del código: la función caliente pasa de 4144 a 3764 B y el lazo acumulador queda como hoja compacta de 680 B en vez de estar embebido entre el fast path y el fallback por-posición. Camino recorrido, por si alguien repite el experimento: la primera versión, con `stride` como variable de runtime en las variables de inducción, midió 492 ms — las capas stride-2 ganaban 21.6 ms pero las stride-1 perdían 10.3; especializar con templates *inline* dio 491 (no recuperó nada); solo `noinline` destapó los 37 ms. Atribución final: V194 504 → 467 (solo hoja `noinline`) → **435** (+ P8 stride-2). Nuevo `EXTRA_DEF` en el Makefile para A/B sin editarlo (`make EXTRA_DEF=-DD2M_NO_S2P8 bench`). Verificado además en QEMU (3 frames, salida limpia) y con `make kernel8.img`. Corolario para el roadmap: la premisa "conv3×3 está cerca del piso del A53" seguía sin sostenerse — el mismo kernel, sin tocar una sola FMA, dio 14 % más. |
| **V196** | **El reloj ARM se caía a 600 MHz a los ~60 s: 705 → 435 ms sostenidos (+38 % de throughput real).** Reportado por el usuario ("los primeros 100 frames a 2 FPS, después 1.2-1.4") y reproducido con `make bench FRAMES=150`: frames 1-95 a 435 ms, frame 100 en adelante a 705 ms, estable. El `[SOC]` de V194 lo resolvió en una corrida: **`arm` pasa de 1000 a 600 MHz**, con `thr=0x00000000` y la temperatura **bajando** (60.1 → 57.9 °C). No es throttle ni calor — es la ventana de turbo inicial del firmware: en Linux el driver cpufreq pide el reloj por el canal de propiedades, bare-metal no lo pide nunca, así que al expirar el VideoCore devuelve el ARM a `arm_freq_min` (600 MHz en el BCM2837). 435 × 1000/600 = 725 ≈ 705, consistente con que el grafo es compute-bound y escala lineal con el reloj (V189). **Fix**: `soc_clock_boost()` en `bsp/mailbox.cpp`, llamado en `kernel_main` tras `init_mmu` — `GET_MAX_CLOCK_RATE` (0x00030004) del clock ARM (id 3) y luego `SET_CLOCK_RATE` (0x00038002) a ese máximo, sin hardcodear 1000 MHz. **Un tag por llamada**: meter `SET_TURBO` (0x00038005) en el mismo buffer hace que este firmware rechace la petición entera con `st=0x80000001` — `SET_CLOCK_RATE` solo devuelve `rc=0x80000008 hz=0x3B9ACA00` y concede el reloj igual. La petición **persiste**: 301 frames a 435-436 ms, hasta 66 °C, `thr=0x0`, el reloj nunca recae. Añadido un re-pedido automático en `soc_status_report` si alguna vez ve el ARM por debajo del máximo (imprime `REBOOST->`; en 450 frames de prueba no se disparó nunca). Sin cambios en la SD — alternativa equivalente por config.txt sería `force_turbo=1` o `arm_freq_min=1000`, pero eso obliga a recopiar la tarjeta y no queda registrado en el binario. **Corrección a V194**: su conclusión "no es throttling" sigue siendo literalmente cierta (`thr` nunca deja de ser 0), pero su "el banco repite a ±0 ms" solo valía **dentro de la ventana de turbo** — se midió sobre 3 frames, ~2 s tras el reset, una muestra ciega a una degradación que empieza en el frame 100. Todas las cifras de V195 (504/467/435) se tomaron dentro de esa ventana a 1000 MHz, así que los A/B entre ellas siguen siendo válidos; lo que no era válido era leerlas como rendimiento sostenido. Desde ahora el bench largo (`FRAMES=150`) es parte de validar cualquier número. |
| **V197** | **Padding explícito: el fast path pasa a cubrir el 100 % de las posiciones — 435 → 367 ms (−15.6 %).** El P8 solo corre donde la ventana 3×3 no toca padding; el anillo de 1 px caía al camino genérico que re-lee los pesos por posición (~2× más lento, según el A/B de V191). En este grafo ese anillo pesa mucho en las rejillas chicas: **6.6 % de las posiciones a S4, 12.9 % a S8, 25 % a S16 y 46.7 % a S32**. En vez de acelerar el borde, se elimina: `pad3x3_chw()` copia la entrada una vez a un buffer `(H+2)×(W+2)` con el borde a cero y la conv corre con **`pad=0`**, donde `lo_lim=0` y `hy_lim=H_out` — no queda ni una posición fuera del fast path y el camino por-posición deja de ejecutarse. Enganchado en `parallel_conv2d` **y** `parallel_conv2d_async_start` (en HW el grafo usa el segundo). Solo K=3/s1/p1: en stride 2 el anillo ya era marginal tras V195 y la copia no compensa. Buffer estático de 512 KB con guard en runtime — si un caso no entra, no padea y sigue por el camino de V195 (más lento, correcto); el mayor caso del grafo a 320×192 es `P3.cls1`, 80 ch a S8 → 87 360 floats. **Medido, y las predicciones por rejilla se cumplen**: P5 head (S32) 25.0 → 16.6 (−34 %, predicho −32), P4 head (S16) 41.3 → 28.5 (−31 %, predicho −20), P3 head (S8) 97.7 → 81.5 (−17 %, predicho −11); por etapa `bb` 152 → 135, `neck` 88 → 74, `head` 163 → 126. Bit-exacto (`[ABS]` 16/16, `[DET]` sin cambio) y sostenido: **201 frames a 366-367 ms**, 66.6 °C, `arm=1000MHz thr=0x0`. Coste asumido de paso: parametrizar `pad` en los templates `p8_accum_*` (era una constante 1) costó 3 ms — 435 → 438 antes de que el padding lo devolviera con creces. Verificado también con `make kernel8.img` y QEMU (3 frames, salida limpia). **Observación anotada, no actuada**: en HW el grafo usa `parallel_conv2d_with_pump` → `parallel_conv2d_async_start`, que reparte entre **cores 1-3** dejando el core 0 bombeando el display (V180). Es decir, todos estos números son de 3 cores; el camino síncrono de 4 cores debería ser ~⅓ más rápido en cómputo, a cambio de congelar el display durante la inferencia. Decisión de diseño abierta, no la toqué. |
| **V198** | **Bug latente en el reparto síncrono de `conv2d`: el último core no tomaba el resto.** Encontrado midiendo cuánto cuesta el reparto async de V180 (cores 1-3) frente al síncrono de 4 cores. El handler de `TASK_CONV2D` hacía `slice = n_grp/4; grp_start = core_id*slice; grp_end = grp_start + slice` — sin el `(core_id == 3) ? n_grp : …` que **sí** tienen `TASK_CONV1X1` y `TASK_CONV2D_ASYNC`. Con `n_grp` no múltiplo de 4 los grupos de cola no los calculaba nadie: `C_out=80` → `n_grp=10`, `10/4=2`, se cubrían 8 grupos y **los canales 64-79 de las tres ramas `cls` (P3/P4/P5) quedaban con basura**. Sobrevivió porque en HW el grafo va por el camino async desde V180 y en QEMU los secundarios no arrancan (fallback single-core), así que el camino síncrono multicore no se ejecutaba en ningún build. Se delató solo al forzarlo: `[DET]` vacío y 2 de 16 checkpoints `[ABS]` fuera de tolerancia — justo `P3_CLS` y `P5_CLS`. Arreglado (una línea). **Medición que lo destapó, ya con el fix**: mismo grafo, `EXTRA_DEF=-DD2M_SYNC_CONV` (nuevo flag que fuerza el camino síncrono fuera del sim) → **264 ms contra los 367 del async**, ambos con `[DET]` y huella PASS. Es decir, dejar el core 0 bombeando el display cuesta **103 ms/frame (28 %)**; el 4/3 teórico serían 92, el resto es el `wfe`/`sev` por conv. Decisión de diseño no tomada: el async existe para que la cámara y el HUD no se congelen durante la inferencia (V180), así que no cambié el default. |
