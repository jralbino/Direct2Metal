    ...
    for (int p_step = 0; p_step < HW; p_step += TILE_P) {
        ...
        for (; co <= C_out_end - 4; co += 4) {
            ...
            for (; p <= p_max - 4; p += 4) {
                ...
                vst1q_f32(out0 + p, acc0); 
                vst1q_f32(out1 + p, acc1); 
                vst1q_f32(out2 + p, acc2); 
                vst1q_f32(out3 + p, acc3);
            }
            for (; p < p_max; p++) {
                ...
                if(do_silu) { 
                    sum0 *= mini_sigmoidf(sum0); 
                    sum1 *= mini_sigmoidf(sum1); 
                    sum2 *= mini_sigmoidf(sum2); 
                    sum3 *= mini_sigmoidf(sum3); 
                }
