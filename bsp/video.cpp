...
static const char k_font_chars[] =
    "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ:.% -";

static const uint8_t k_font[41][7] = {
...
static int font_index(char c) {
    /* Convert lowercase letters to uppercase so class names like "person"
     * resolve to glyphs in the uppercase-only table. */
    if (c >= 'a' && c <= 'z') c = (char)(c - 32);
    for (int i = 0; k_font_chars[i]; i++)
        if (k_font_chars[i] == c) return i;
    return 39;  // unknown → space (index of ' ' in k_font_chars)
}
